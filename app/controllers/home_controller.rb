class HomeController < ApplicationController
  ...
  def advance
    ...
    if params[:val] == "person"
      if params[:arrow] == "up"
        @ideas=Idea.paginate(:page => params[:page], :per_page => 10).sort{|x,y| x.title <=> y.title}
      elsif params[:arrow] == "down"
        @ideas=Idea.paginate(:page => params[:page], :per_page => 10).sort{|x,y| y.title <=> x.title}
      end
      ...
      if params[:arrow] == "up"
        @ideas=Idea.paginate(:page => params[:page], :per_page => 10, :order => "ideas.comment_count DESC")
      elsif params[:arrow] == "down"
        @ideas=Idea.paginate(:page => params[:page], :per_page => 10, :order => "ideas.comment_count ASC")
      end
    else
