...
#options[:select] << " o w w e"          
          ...
          if is_postgresql_83?
            from_part = "plainto_tsquery('#{search_string}') as tsearch_query"
          else
            from_part = "plainto_tsquery('#{locale}','#{search_string}') as tsearch_query"
          end
          if options[:from]
        ...
        # [Patched] Change regex to not match empty double quotes
